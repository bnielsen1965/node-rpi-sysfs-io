...
class RPiSysfsIO {
  ...
  async _exportGPIO (gpio, waitExport) {
    if (! await this.isExportedGPIO(gpio)) await this.write(this.settings.sysfsGPIOPath + 'export', gpio);
    if (!waitExport) return;
  ...
  async _waitExported (path) {
    let waitTime = 0;
    do {
      if (await this.isExported(path) && await this.haveAccess(path)) return;
      await this.wait(this.settings.exportWaitInterval);
      waitTime += this.settings.exportWaitInterval;
    } while (waitTime <= this.settings.exportWaitTimeout);
